...
describe("Percentile tsx", () => {
  it("renders 50%", () => {
    ...
    render(<PercentileViewer data={data} score={score} />)
  ...
  it("renders transition from 0% to 100%", () => {
    ...
    const { rerender } = render(<PercentileViewer data={data} score={0.0} />)
    expect(screen.getByText("0%")).toBeTruthy()
    rerender(<PercentileViewer data={data} score={1.5} />)
    expect(screen.getByText("50%")).toBeTruthy()
    rerender(<PercentileViewer data={data} score={3.0} />)
    expect(screen.getByText("100%")).toBeTruthy()
    rerender(<PercentileViewer data={data} score={1.5} />)
    expect(screen.getByText("50%")).toBeTruthy()
  })
